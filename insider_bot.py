# ============================================================================
# INSIDER TRADING BOT v2.1 - WORKING VERSION
# ============================================================================
import os
import time
import json
...
KELLY_FRACTION = 0.10
ROLLING_WINDOW = 200
MAX_FILING_DELAY = 10  # FIXED: 10 days to match backtest
MIN_PRICE = 1.00
MAX_PRICE = 10000
MIN_BOT_PRICE = 10.00
...
class BotState:
    ...
    def save(self):
        with open(self.state_file, 'w') as f:
            json.dump({'trade_history': self.trade_history, 'open_positions': self.open_positions}, f, indent=2)
        logger.info("Saved")
...
def get_stop_price(entry, highest):
    return (highest if highest > entry else entry) * (1 - STOP_LOSS_PCT)
...
def fetch_filings(target_date):
    try:
        return get_filings(form="4", filing_date=target_date.strftime('%Y-%m-%d'))
...
def process_filings(filings, target_date):
    ...
    if stats['bad_ticker'] > 0:
        logger.info(f"Filtered {stats['bad_ticker']} NONE tickers")
    if stats['bad_delay'] > 0:
        logger.info(f"Filtered {stats['bad_delay']} trades with delay > {MAX_FILING_DELAY} days")
    
    return unique

# ============================================================================
# PLACE ORDERS - FRACTIONAL SHARES
# ============================================================================
...
def run_bot():
    logger.info("="*60)
    logger.info("🚀 INSIDER TRADING BOT v2.1 - FRACTIONAL SHARES")
    logger.info(f"Filing Delay: {MAX_FILING_DELAY} days")
    logger.info("="*60)
